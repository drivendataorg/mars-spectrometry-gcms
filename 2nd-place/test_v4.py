...
@click.option("--results_dir", type=str,
              ...
              # default='~/tmp/ddm_v4_outputs',  # rerun
              default='~/tmp/ddm_2nd_place_weights',  # e.g. from unzipped
              # default='~/tmp/ddm_2nd_place_weights_dbg',  # rerun of predict
...
def export_test_preds(best_try_list=None):
    ...
    best_models = sorted([tuple(main_cfg.check_models[i]) for i in best_try_list])
    ...
    # out_path = f'test_results_all/{fname}'
    os.makedirs(dirname(out_path), exist_ok=True)
